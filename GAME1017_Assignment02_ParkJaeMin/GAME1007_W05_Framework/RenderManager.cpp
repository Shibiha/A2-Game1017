...
{
	m_pWindow = SDL_CreateWindow(title, xPos, yPos, width, height, wFlags); // creating window
	if (m_pWindow == nullptr)
	{
		std::cout << "Error during window creation!" << std::endl; // error during window creation
		return 1;
	}
	m_pRenderer = SDL_CreateRenderer(m_pWindow, -1, rFlags);
	if (m_pRenderer == nullptr)
	{
		std::cout << "Error during renderer creation!" << std::endl; // error during rendering
		return 1;
	}
	std::cout << "RenderManager init success!" << std::endl;
	return 0; // Meaning successful creation.
}

void RenderManager::Quit() // destroy
{
